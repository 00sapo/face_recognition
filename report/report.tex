...
\begin{itemize}
	...
	\item \textit{leave-one-out}: only on \verb|dataset a|, without
...
\subsection{SVM prediction}\label{prediction}
...
\\
Table \ref{tab:results-alpha} shows that with this rule \textit{FP-unknown} is
always greater or equal to the sum of \textit{FP-unknown} obtained using RGB
imges only or depth images only. Also, it shows that RGB images have always
\textit{FP-unknown} equal to $0$. Because of this we changed the rule so that
it used RGB votes only (while using depth images only, the rule checked
the only votes from depth SVM). In the definitive version, $t = c$.
